...
- **v1.3 → v1.4** : Ajout lecture `.claude/instructions/orchestrator.instructions.md` au démarrage pour appliquer les spécificités projet MAINa.
...
- **v2.2 → v2.3** : Index plans simplifié (sans phases) + màj obligatoire `.claude/plans/README.md` lors changement statut plan.
...
- **v2.4 → v2.5** : Extraction procédures Plans Action et /fleet en skills partagés (`.claude/skills/`). Sections AP et /fleet réduites aux spécificités ARCos (orchestration, création plan).
- **v2.5 → v2.6** : Alignement sur nouvelle arborescence vrais skills (`.claude/skills/<nom>/SKILL.md`).
- **v2.6 → v2.7** : Ajout skill `adr-writing` (`.claude/skills/adr-writing/SKILL.md`). ARCos prépare contenu ADR, DOCly rédige fichier. Référence explicite skill après accord humain sur solution.
...
- **v2.0 → v2.1** : Ajout règle synchro obligatoire `.claude/plans/README.md` (index plans + statut global uniquement).
- **v2.1 → v2.2** : Extraction procédures Plans Action et /fleet en skills partagés (`.claude/skills/`). Section AP réduite aux spécificités DEVon.
- **v2.2 → v2.3** : Alignement sur nouvelle arborescence vrais skills (`.claude/skills/<nom>/SKILL.md`).
...
- **v1.9 → v2.0** : Ajout instruction parallélisation avec /fleet.
- **v2.1 → v2.2** : Déplacement validations QA spécifiques projet vers `.claude/instructions/qa.instructions.md`.
- **v2.2 → v2.3** : Ajout synchronisation obligatoire `.claude/plans/README.md` lors changements statut plan.
- **v2.3 → v2.4** : Extraction procédures Plans d'Action et /fleet en skills partagés (`.claude/skills/`). Section AP réduite aux spécificités QALvin.
- **v2.4 → v2.5** : Alignement sur nouvelle arborescence vrais skills (`.claude/skills/<nom>/SKILL.md`).
...
- **v2.1 → v2.2** : Ajout règle maintenance `.claude/plans/README.md` (index plans + statut global seulement).
- **v2.2 → v2.3** : Extraction procédures Plans d'Action + /fleet en skills partagés (`.claude/skills/`). Section AP réduite aux spécificités DOCly.
- **v2.3 → v2.4** : Alignement nouvelle arbo vrais skills (`.claude/skills/<nom>/SKILL.md`).
