...
Règle applique **tous agents + Claude**. Sans exception, sans dérogation.
...
- **Jamais accéder** ressources : lecture, écriture, exécution, inclusion, référence indirecte, grep, glob, analyse statique
...
1. Vérifier existence `.copilotignore` à racine projet
2. Si oui : **lire uniquement liste patterns** (fichier `.copilotignore` lui-même), jamais fichiers désignés
