...
#### T5.2 - Spike upgrade sur module isolé (portée révisée : Java 25/Mandrel 25 sur Quarkus 3.37.1, PAS Quarkus 4.x)
- **Agent :** DEVon
- **Fichier(s) :** `parametrages` (module le plus petit) + `communs`, sur branche/worktree dédiée
- **Couvrir :** upgrade `maven.compiler.release` 21→25 + Mandrel 25, Quarkus **reste 3.37.1** (pas de changement de ligne majeure — risque réduit vs plan initial). Vérifier hints GraalVM reflection existants (4 fichiers `JwtReflectionConfig.java`) toujours valides sous Mandrel 25
- **Acceptation :** liste de breaking changes (probablement minimes, simple bump JDK) + effort réel estimé
