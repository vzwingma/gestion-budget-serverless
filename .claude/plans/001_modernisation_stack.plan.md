...
**Statut :** ✅ Phases 1-4, 6, 7 complétées (ADR-001/002, upgrade Quarkus 3.37.1, migrations Mongo maison, tuning infra SAM/CI, .gitignore, architect.instructions.md enrichi, mesure MemorySize réelle 256Mo confirmé, fix SonarCloud PR #186, sed cosmétique corrigé, check CI désync version Quarkus). ✅ **QUA et PROD déployés et fonctionnels** (2026-07-07) après résolution de 2 incidents réels (stack bloquée par renommage LogicalId Phase 3 + CORS cassé par valeurs GitHub pré-échappées — détail note Phase 3). ✅ Phase 8 : T8.2 complétée (couverture réelle `MigrationRepository`, bug générique `ObjectId`/`String` trouvé et corrigé) — **T8.1 reste bloquée** (backport Quarkus #55278 pas encore disponible). **Phase 5** (Quarkus 4.x/Java 25) débloquée en principe (Phase 2 stable en prod réelle) — à relancer en session dédiée si souhaité.
**Porteur :** ⚫ MAINa
