...
- [`003_remediation_sonar.plan.md`](./003_remediation_sonar.plan.md) — 🔵 **En cours (Phase A)**. Remédiation 314 issues SonarCloud OPEN (`vzwingma_gestion-budget-serverless`). Phase A (blocker + majeurs `MongoMigrationRunner.java`, fichier partagé 4 microservices, 9 issues) démarrée : DEVon (T A.1-A.3) → Gate #2 → QALvin → Gate #3. Phases B (S8688 `.now()` UTC, ADR-004 requis), C (S8924 imports statiques Mockito, 267 issues) et D (reliquats) différées, attendent nouvelle sollicitation.
