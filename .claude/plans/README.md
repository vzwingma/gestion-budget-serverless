...
- [`003_remediation_sonar.plan.md`](./003_remediation_sonar.plan.md) — 🔵 **En cours (Phase A complétée ; Phase B — T B.1 `communs` complétée, T B.2 en attente republication `communs` + feu vert explicite ; C/D en attente)**. Remédiation 314 issues SonarCloud OPEN (`vzwingma_gestion-budget-serverless`). Phase A (blocker + majeurs `MongoMigrationRunner.java`) ✅ complétée et **committée/poussée** (branche `feat/sonar_mcp`, commits `ffa8fdd`+`d449923`) : 8/9 issues corrigées (S125 introuvable), tests ciblés 11/11 verts, `mvn clean test` racine 142/146 (4 erreurs pré-existantes sans lien, validation complète différée scan CI `master`). Phase B (S8688 `.now()` UTC, 18 issues) : [`docs/adr/004-clock-injection-convention.md`](../../docs/adr/004-clock-injection-convention.md) produit (ARCos+DOCly), validé. T B.1 (`communs`, 5 call sites) ✅ complétée — `mvn clean test -f communs/pom.xml` 149 tests/0 failure (4 erreurs infra pré-existantes), compilation 4 microservices confirmée OK par QALvin, Gates #2/#3 obtenues. **T B.2 (`operations`+`utilisateurs`, 13 call sites) en attente republication `communs` (GitHub Packages, pipeline CI) + sollicitation explicite utilisateur — non démarrée.** Phases C (S8924 imports statiques Mockito, 267 issues) et D (reliquats) toujours différées.
...
**Dernière mise à jour :** 2026-07-10 (Plan 003 T B.1 `communs` complétée, T B.2 en attente republication `communs` + feu vert explicite)
