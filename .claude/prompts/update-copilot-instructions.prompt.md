...
description: >
  ...
  amender le fichier CLAUDE.md. Utiliser quand : "mets à jour les
  instructions Claude", "complète copilot-instructions depuis le code",
  "synchronise les instructions avec le projet", "ajoute les bonnes pratiques
  aux instructions Claude".
agent: agent
tools:
  - read_file
  - file_search
...
Mission: auditer code source et fichiers référence, puis compléter/amender `.claude/CLAUDE.md` pour refléter état réel projet et bonnes pratiques.
...
Lire intégralement `.claude/CLAUDE.md` pour identifier:
...
Pour chaque convention documentée dans `CLAUDE.md`:
...
Lire 4 skills suivants dans `.claude/skills/` (si existent):
...
- Contenu cohérent avec `.claude/PLANS.md` (pas divergence format)
- Agents `.claude/agents/*.agent.md` référencent skills dans sections AP et /fleet (et répètent pas contenu)
...
3. **Rester concis**: instructions Claude lues chaque session; éviter verbosité
...
7. **Pas dupliquer** infos déjà présentes dans fichiers agents (`.claude/agents/`)
...
1. Présenter **diff résumé** changements proposés:
   ...
   - Vérification agents `.claude/agents/*.agent.md` à version courante (v3.0+)
   - Vérification skills `.claude/skills/*/SKILL.md` présents et cohérents avec `PLANS.md`
   - Modifications proposées pour chaque fichier `.claude/instructions/*.instructions.md` (créés depuis `*.instructions.template.md` si absents)
...
3. Une fois validé, appliquer changements dans `.claude/CLAUDE.md` et, si nécessaire, dans fichiers `.claude/instructions/*.instructions.md` avec `replace_string_in_file` ou `multi_replace_string_in_file`.
