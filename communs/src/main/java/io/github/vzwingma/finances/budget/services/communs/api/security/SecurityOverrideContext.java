...
public class SecurityOverrideContext implements SecurityContext {
    ...
    private final JWTAuthToken jwtToken;
    private final String rawBase64Token;

    /**
     * Constructeur de SecurityOverrideContext.
     *
     * @param jwtToken       Le token JWT utilisé pour l'authentification de l'utilisateur.
     * @param rawBase64Token Le token JWT sous forme de chaîne en Base64.
     */
    public SecurityOverrideContext(JWTAuthToken jwtToken, String rawBase64Token) {
    ...
    public Principal getUserPrincipal() {
        if (jwtToken != null && jwtToken.isValid()) {
            JWTAuthPayload p = this.jwtToken.getPayload();
            if (p != null) {
