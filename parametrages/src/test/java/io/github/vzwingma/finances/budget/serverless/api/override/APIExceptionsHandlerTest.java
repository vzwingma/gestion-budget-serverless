package io.github.vzwingma.finances.budget.serverless.api.override;

import io.github.vzwingma.finances.budget.serverless.services.parametrages.api.override.APIExceptionsHandler;
...
/**
 * Tests unitaires de APIExceptionsHandler (module parametrages)
 */
class APIExceptionsHandlerTest {
    ...
    void testBudgetNotFoundException() {
        try (Response r = handler.toResponse(new BudgetNotFoundException("budget not found"))) {
            assertEquals(Response.Status.NOT_FOUND.getStatusCode(), r.getStatus());
        }
    }
    ...
    void testCompteClosedException() {
        try (Response r = handler.toResponse(new CompteClosedException("compte fermé"))) {
